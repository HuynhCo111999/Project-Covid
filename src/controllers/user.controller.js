...
const orderDetailNecessity = require("../models/index").orderDetailNecessity;
...
exports.getModerators = async(req, res) => {
    ...
    let filterUsers = await listUsers.filter(
        (item) => item["roles.name"] === "moderator"
    );
...
exports.getPersonalInformation = async(req, res) => {
    if (req.cookies.userId) {
        try {
            ...
            const statusId =
                infoUser.histoty_user_statuses[
                    infoUser.histoty_user_statuses.length - 1
                ].statusCovidUserId;
            const locationId =
                infoUser.histoty_user_locations[
                    infoUser.histoty_user_locations.length - 1
        ...
        if (orders.length <= 0) {
            ...
            for (let i = 0; i < orders.length; i++) {
                ...
                for (let j = 0; j < orderDetails.length; j++) {
                    ...
                    descriptionOrder.push(
                        `${necessityCombo.name} (x${orderDetails[j].quantity})`
                    );
                }
...
exports.getBuyNecessityCombo = async(req, res) => {
    ...
    for (let necessityOfCombo of necessityOfCombos) {
        ...
        if (flag == -1) {
            ...
            dataNeccessityCombo[flag].price +=
                necessityOfCombo.min_limit * tempNecessity.price;
...
exports.postAddCart = async(req, res) => {
    ...
    for (let i = 0; i < necessities.length; i++) {
        priceCombo +=
            necessities[i].price_necessity * necessities[i].quantity_necessity;
        descriptionArray.push(
            `${i + 1}. ${necessities[i].quantity_necessity} ${
        necessities[i].unit_of_measurement
      } ${necessities[i].name_necessity} (${necessities[i].price_necessity}đ/${
        necessities[i].unit_of_measurement
      })`
        );
    }
...
exports.postOrderNecessityCombo = async(req, res) => {
        ...
        for (let j = 0; j < idCombos.length; j++) {
            if (req.session.cart[i].idCombo == idCombos[j]) {
                ...
                req.session.cart[i].amount =
                    req.session.cart[i].quantity * req.session.cart[i].price;
            }
        }
    }
    ...
    if (orders.length <= 0) {
        ...
        for (let i = 0; i < req.session.cart.length; i++) {
            ...
            let dayDivide;
            if (req.session.cart[i].sales_cycle === null) {
                // theo ngày
                currentDate = currentDay;
                dayDivide = 1;
            } else if (req.session.cart[i].sales_cycle === false) {
                // theo tuần
                currentDate = currentWeek;
                dayDivide = 7;
            } else if (req.session.cart[i].sales_cycle === true) {
                // theo tháng
            ...
            for (let j = 0; j < orders.length; j++) {
                ...
                if (Difference_In_Time > 0) {
                    let Difference_In_Days =
                        Difference_In_Time / (1000 * 3600 * 24 * dayDivide);
        ...
        for (let i = 0; i < req.session.cart.length; i++) {
            ...
            quantityRemainSubtract.push(
                quantityRemain[i] - req.session.cart[i].quantity
            );
